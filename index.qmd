...
# Create the plot with more granular bins
p <- ggplot(simulation_results, aes(x = final_balance)) +
  geom_histogram(bins = 40, fill = "steelblue", alpha = 0.7, color = "black", linewidth = 0.2) +
  geom_density(aes(y = after_stat(count) * 1.5), color = "darkblue", linewidth = 1, alpha = 0.8) +
  ...
  geom_vline(xintercept = median_balance, color = "orange", linetype = "dotdash", linewidth = 1) +
  labs(
    title = "Distribution of Final Account Balances",
    subtitle = "100 Simulations: Age 20 to 55 (40 bins for granular view)",
    x = "Final Balance at Age 55 ($)",
    y = "Frequency",
    caption = "Red dashed = Initial $1,000 | Green solid = Mean | Orange dash-dot = Median"
  ) +
  scale_x_continuous(labels = scales::dollar_format(), breaks = scales::pretty_breaks(n = 10)) +
