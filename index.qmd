...
At first glance, the game looks appealing: the expected value is $1,050, or a 5% gain per flip. But that number only tells you the average if the game is played countless times. In reality, because each flip compounds, a bad run can shrink your balance fast, while a good run can boost it just as quickly. The real takeaway is that expected value shows the long-run average, but your actual outcome depends entirely on the sequence you get making the game much riskier than it first appears.
...
On paper, the game looks strong: the expected value is $1,050, or a 5% gain per flip. That suggests players should come out ahead on average. But averages only hold when you can play thousands of times. In reality, you only get one sequence of flips until age 55, and that exposes you to the full volatility. A few losses can drag your balance down sharply, while a lucky run could send it soaring. The real lesson is that expected value shows the average, not the risk. In high-variance situations like this, you have to think about the whole range of possible outcomes—not just the appealing average.
...
# Create visually appealing probability distribution plot
library(ggplot2)
library(scales)
...
p <- ggplot(simulation_results, aes(x = final_balance)) +
  ...
  geom_histogram(aes(fill = after_stat(count)), bins = 25, alpha = 0.8, color = "white", linewidth = 0.3) +
  # Add density curve overlay
  geom_density(aes(y = after_stat(count) * 2), color = "darkblue", linewidth = 1.2, alpha = 0.7) +
  # Reference lines with enhanced styling
  geom_vline(xintercept = initial_balance, color = "#e74c3c", linetype = "dashed", linewidth = 1.5, alpha = 0.8) +
  geom_vline(xintercept = mean_balance, color = "#27ae60", linetype = "solid", linewidth = 1.5, alpha = 0.8) +
  geom_vline(xintercept = median_balance, color = "#f39c12", linetype = "dotdash", linewidth = 1.2, alpha = 0.8) +
  # Enhanced labels and styling
  labs(
    title = "Investment Game: Final Balance Distribution",
    subtitle = "100 Monte Carlo Simulations | Age 20 to 55",
    x = "Final Balance at Age 55 ($)",
    y = "Number of Simulations",
    caption = "Red dashed = Initial $1,000 | Green solid = Mean | Orange dash-dot = Median",
    fill = "Count"
  ) +
  # Enhanced scales and formatting
  scale_x_continuous(
    labels = dollar_format(scale = 1, prefix = "$", suffix = ""),
    breaks = pretty_breaks(n = 8),
    trans = "log10"
  ) +
  scale_fill_gradient(low = "#3498db", high = "#2980b9", guide = "none") +
  # Professional theme
  theme_minimal() +
  theme(
    plot.title = element_text(size = 16, face = "bold", color = "#2c3e50", margin = margin(b = 10)),
  ...
  annotate("text", x = initial_balance * 1.2, y = max(table(cut(simulation_results$final_balance, 25))) * 0.8, 
           label = paste("Initial\n$1,000"), color = "#e74c3c", fontface = "bold", size = 3.5) +
  annotate("text", x = mean_balance * 1.3, y = max(table(cut(simulation_results$final_balance, 25))) * 0.6, 
           label = paste("Mean\n$", round(mean_balance, 0)), color = "#27ae60", fontface = "bold", size = 3.5) +
  annotate("text", x = median_balance * 0.7, y = max(table(cut(simulation_results$final_balance, 25))) * 0.4, 
           label = paste("Median\n$", round(median_balance, 0)), color = "#f39c12", fontface = "bold", size = 3.5)
...
cat("  Point estimate: ", round(prob_above_1000 * 100, 1), "%\n", sep = "")
cat("  95% Confidence Interval: [", round(ci_lower * 100, 1), "%, ", round(ci_upper * 100, 1), "%]\n", sep = "")
cat("  Number of simulations above $1,000: ", sum(final_balances > 1000), " out of ", n_sims, "\n", sep = "")

cat("\nAdditional Probability Thresholds:\n")
cat("  Probability > $10,000: ", round(prob_above_10000 * 100, 1), "%\n", sep = "")
cat("  Probability > $100,000: ", round(prob_above_100000 * 100, 1), "%\n", sep = "")

cat("\nInterpretation:\n")
if (prob_above_1000 > 0.5) {
  cat("  ✓ More likely than not to end up above $1,000\n")
} else {
  cat("  ✗ More likely to end up at or below $1,000\n")
}

cat("\nRisk Assessment:\n")
cat("  Probability of losing money: ", round(prob_below_1000 * 100, 1), "%\n", sep = "")
cat("  Expected final balance: $", round(mean(final_balances), 2), "\n", sep = "")
cat("  Median final balance: $", round(median(final_balances), 2), "\n", sep = "")

# Show distribution summary
cat("\nDistribution Summary:\n")
cat("  Minimum: $", round(min(final_balances), 2), "\n", sep = "")
cat("  Maximum: $", round(max(final_balances), 2), "\n", sep = "")
cat("  Standard deviation: $", round(sd(final_balances), 2), "\n", sep = "")
```
...
cat("ORIGINAL STRATEGY (100% of balance at risk):\n")
cat("  Mean final balance: $", round(orig_mean, 2), "\n", sep = "")
cat("  Median final balance: $", round(orig_median, 2), "\n", sep = "")
cat("  Standard deviation: $", round(orig_sd, 2), "\n", sep = "")
cat("  Probability > $1,000: ", round(orig_prob_above_1000 * 100, 1), "%\n", sep = "")
cat("  Probability > $10,000: ", round(orig_prob_above_10000 * 100, 1), "%\n", sep = "")
cat("  Min balance: $", round(min(orig_final_balances), 2), "\n", sep = "")
cat("  Max balance: $", round(max(orig_final_balances), 2), "\n", sep = "")

cat("\nMODIFIED STRATEGY (50% of balance at risk):\n")
cat("  Mean final balance: $", round(mod_mean, 2), "\n", sep = "")
cat("  Median final balance: $", round(mod_median, 2), "\n", sep = "")
cat("  Standard deviation: $", round(mod_sd, 2), "\n", sep = "")
cat("  Probability > $1,000: ", round(mod_prob_above_1000 * 100, 1), "%\n", sep = "")
cat("  Probability > $10,000: ", round(mod_prob_above_10000 * 100, 1), "%\n", sep = "")
cat("  Min balance: $", round(min(mod_final_balances), 2), "\n", sep = "")
cat("  Max balance: $", round(max(mod_final_balances), 2), "\n", sep = "")
...
cat("\nSTATISTICAL COMPARISON:\n")
cat("======================\n")
cat("Difference in mean final balance: $", round(mod_mean - orig_mean, 2), "\n", sep = "")
cat("Difference in probability > $1,000: ", round((mod_prob_above_1000 - orig_prob_above_1000) * 100, 1), " percentage points\n", sep = "")
cat("Difference in probability > $10,000: ", round((mod_prob_above_10000 - orig_prob_above_10000) * 100, 1), " percentage points\n", sep = "")

# Risk assessment
cat("\nRISK ASSESSMENT:\n")
cat("================\n")
cat("Original strategy volatility (CV): ", round(orig_sd / orig_mean, 3), "\n", sep = "")
cat("Modified strategy volatility (CV): ", round(mod_sd / mod_mean, 3), "\n", sep = "")

if (mod_sd / mod_mean < orig_sd / orig_mean) {
  cat("✓ Modified strategy has LOWER relative volatility\n")
} else {
  cat("✗ Modified strategy has HIGHER relative volatility\n")
}

cat("\nCONCLUSION:\n")
cat("===========\n")
if (mod_prob_above_10000 > orig_prob_above_10000) {
  cat("✓ Modified strategy has HIGHER probability of reaching $10,000+\n")
} else {
  cat("✗ Modified strategy has LOWER probability of reaching $10,000+\n")
}

if (mod_prob_above_1000 > orig_prob_above_1000) {
  cat("✓ Modified strategy has HIGHER probability of staying above $1,000\n")
} else {
  cat("✗ Modified strategy has LOWER probability of staying above $1,000\n")
}
```
